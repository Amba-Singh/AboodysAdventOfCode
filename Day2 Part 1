#PART 1 (UNTIL LINE 160)
import json
mainstring = '''{Game 1: 12 blue; 2 green, 13 blue, 19 red; 13 red, 3 green, 14 blue
...
primarystring = mainstring.replace(' ', '')
secondarystring = primarystring.replace(',', '')
tertiarystring = secondarystring.replace(';','')
quartenarystring = tertiarystring.replace('lue','$')
quinarystring = quartenarystring.replace('reen','$')
senarystring = quinarystring.replace('ed','$')
septenarystring = senarystring.replace('\n', '",')
octonarystring = septenarystring.replace('G','"G')
nonarystring = octonarystring.replace(':','":"')

deletedSum = 0
diction = json.loads(nonarystring)
print(diction)

for game in diction:
    condition = False
    temp = diction[game].split('$')
    print(temp)
    blueList = []
    for x in temp:
        if x.__contains__('b'):
            x = x.replace('b','')
            blueList.append(int(x))
    print(blueList)
    for integ in blueList:
        if integ > 14:
            deletedSum += int(game.replace('Game',''))
            condition = True
            break
    ...
    for x in temp:
        if x.__contains__('g'):
            x = x.replace('g', '')
            greenList.append(int(x))
    print(greenList)
    for integ in greenList:
        if integ > 13 and condition == False:
            deletedSum += int(game.replace('Game', ''))
            condition = True
            break
    ...
    for x in temp:
        if x.__contains__('r'):
            x = x.replace('r', '')
            redList.append(int(x))
    print(redList)
    for integ in redList:
        if integ > 12 and condition == False:
            deletedSum += int(game.replace('Game', ''))
            break
print(5050 - deletedSum)
...
for game in diction:
    ...
    for x in temp:
        if x.__contains__('r'):
            ...
            redList.append(int(x))
    print(redList)
    highestRed = max(redList)
    for integ in redList:
        if integ > 12 and condition == False:
            deletedSum += int(game.replace('Game', ''))
            break
    product = highestRed * highestGreen * highestBlue
